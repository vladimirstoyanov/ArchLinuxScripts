...
package = Package()
list_packages = package.getVulnerablePackagesInfomation()
list_local_packages = package.getLocalPackagesInformation()
...
for i in range (len(list_packages)):
    for j in range(len(list_local_packages)):
        if (list_packages[i][0] == list_local_packages[j][0]):
            if (package.compareVersions(list_packages[i][1],list_local_packages[j][1])):
